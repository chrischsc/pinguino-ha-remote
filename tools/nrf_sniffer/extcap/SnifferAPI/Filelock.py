...
def _lockdir():
    # FHS lock dir if writable, else a user-writable fallback (XDG runtime / tmp).
    for d in ('/var/lock', '/run/lock', os.environ.get('XDG_RUNTIME_DIR'), '/tmp'):
